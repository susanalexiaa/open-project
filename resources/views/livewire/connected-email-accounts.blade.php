<div class="bg-white bg-opacity-25 p-6">
    <div class="mt-10 sm:mt-0">
        <x-jet-action-section>
                    ...
                    <div class="space-y-6">
                        ...
                        @else
                            @foreach ($accounts as $account)
                                <div class="flex items-center justify-between border-b-1 border-gray-300 pb-2">
                                    <div class="flex items-center">
                                        <div class="ml-4">{{ $account->name }}</div>
                                    </div>
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <div class="flex-initial px-2 text-right">
                                            <x-jet-button wire:click="updateShowModal('{{ $account->id }}')" wire:loading.attr="disabled">
                                                {{ __('Update') }}
                                            </x-jet-button>
                                        </div>
                                        <div class="flex-initial px-2 text-right">
                                            <x-jet-danger-button wire:click="deleteShowModal('{{ $account->id }}')" wire:loading.attr="disabled">
                                                {{ __('Delete') }}
                                            </x-jet-danger-button>
                                        </div>
                                    </div>
            ...
            <x-slot name="actions">
                ...
                <x-jet-button wire:loading.attr="disabled" wire:click="showCreatePopup">
                    {{ __('Add') }}
                </x-jet-button>
            </x-slot>
        </x-jet-action-section>
    ...
    <x-jet-dialog-modal wire:model="showCreatePopup">
        <x-slot name="title">
            {{ __('Create email connection') }}
        </x-slot>
        ...
        <x-slot name="content">
            ...
            <div class="mt-4">
                ...
                <select id="operator" wire:model.defer="operator_id" class="select max-w-xs bg-gray-200">
        ...
        <x-slot name="footer">
            <x-jet-secondary-button wire:click="$toggle('showCreatePopup')" wire:loading.attr="disabled">
                {{ __('Exit') }}
